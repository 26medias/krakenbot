...
class KrakenMonitor {
    constructor(options = {}) {
        ...
        this.pingIntervalMs = Number.isFinite(options.pingIntervalMs) && options.pingIntervalMs > 0
            ? options.pingIntervalMs
            : 15_000;
        this._publicPingTimer = null;
    ...
    async _closeSocket(socket) {
        ...
        if (socket === this.publicSocket) {
            this._stopPublicPing();
        }
        if (socket === this.privateSocket) {
            this._stopPrivatePing();
        }
    }
    ...
    _startPublicPing() {
        ...
        this._publicPingTimer = setInterval(() => {
            if (this.publicSocket && this.publicSocket.readyState === WebSocket.OPEN) {
                try {
                    this.publicSocket.send(JSON.stringify({ method: 'ping' }));
                } catch (error) {
                    this.logger.debug && this.logger.debug('Public ping failed:', error.message);
                }
            }
        }, this.pingIntervalMs);
        if (typeof this._publicPingTimer.unref === 'function') {
            this._publicPingTimer.unref();
        }
    }

    _stopPublicPing() {
        if (this._publicPingTimer) {
            clearInterval(this._publicPingTimer);
            this._publicPingTimer = null;
        }
    }
    ...
    _startPrivatePing() {
        ...
        this._privatePingTimer = setInterval(() => {
            if (this.privateSocket && this.privateSocket.readyState === WebSocket.OPEN) {
                try {
                    this.privateSocket.send(JSON.stringify({ method: 'ping' }));
                } catch (error) {
                    this.logger.debug && this.logger.debug('Private ping failed:', error.message);
                }
            }
        }, this.pingIntervalMs);
        if (typeof this._privatePingTimer.unref === 'function') {
            this._privatePingTimer.unref();
        }
    }
